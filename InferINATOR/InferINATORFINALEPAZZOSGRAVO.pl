...
infer_class_membership :-
    forall(triple(X, rdfs:subClassOf, Y),
           ((\+ triple(X, rdf:type, owl:'Class') -> assert(triple(X, rdf:type, owl:'Class')),
                format("Inferred and asserted that ~w is a class.\n", [X]); true),
            (\+ triple(Y, rdf:type, owl:'Class') -> assert(triple(Y, rdf:type, owl:'Class')),
                format("Inferred and asserted that ~w is a class.\n", [Y]); true))).

% Inferisce il tipo del soggetto basato sul rdfs:domain della proprietà
infer_property_domain :-
    forall((triple(S, P, _), is_property(P), triple(P, rdfs:domain, C)),
           ((\+ triple(S, rdf:type, C) -> assert(triple(S, rdf:type, C)),
...
infer_disjoint_individuals_on_individual :-
    findall((IndividualA, IndividualB),
            (triple(IndividualA, owl:differentFrom, IndividualB), triple(IndividualA, owl:sameAs, IndividualB)),
            Inconsistencies),
...
report_and_remove_different_inconsistencies_on_individual([(IndividualA, IndividualB) | Rest]) :-
    ...
    (triple(IndividualA, owl:differentFrom, IndividualB) -> retract(triple(IndividualA, owl:differentFrom, IndividualB)); true),
    (triple(IndividualB, owl:differentFrom, IndividualA) -> retract(triple(IndividualB, owl:differentFrom, IndividualA)); true),
    (triple(IndividualA, owl:sameAs, IndividualB) -> retract(triple(IndividualA, owl:sameAs, IndividualB)); true),
    (triple(IndividualB, owl:sameAs, IndividualA) -> retract(triple(IndividualB, owl:sameAs, IndividualA)); true),
